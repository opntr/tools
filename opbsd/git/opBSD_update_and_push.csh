...
setenv REPLYTO "op@hardenedbsd.org"

set OPWD=`pwd`
set SOURCE_DIR="/usr/data/source/git/opBSD"
set BRANCHES=`cat $SOURCE_DIR/opbsd/git/opBSD_branches.txt`
set SOURCE="$SOURCE_DIR/opBSD.git"
set LOGS="$HOME/log/opBSD"
set DATE=`date "+%Y%m%d%H%M%S"`
set TEE_CMD="tee -a"
set LOCK="${SOURCE_DIR}/opbsd-repo-lock"
...
handle_err:
	...
	if ( ${ENABLE_MAIL} == "YES" ) then
		cat ${LOGS}/${_branch}-${DATE}.log | \
		    mail -s "[opBSD]${_mail_subject_prefix} ${_branch}-${DATE}.log" ${DST_MAIL}
	endif
	echo
end
